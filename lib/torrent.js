...
function Torrent(torrentId, client, opts) {
  ...
  this.neededFile = opts.neededFile
...
Torrent.prototype._processParsedTorrent = function (parsedTorrent) {
  ...
  if (this.neededFile) {
    // Allow filtering of to-be-downloaded files
    // parsedTorrent.files = parsedTorrent.files.map(file => {
    //   file.selected = this.neededFile(file)
    //   return file
    // })
    parsedTorrent.files = parsedTorrent.files.filter(this.neededFile)
    parsedTorrent.length = parsedTorrent.files.reduce((c, f) => c + f.length, 0)
  }
